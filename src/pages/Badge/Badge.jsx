...
export default function Badge() {
  ...
  const unlockBadge = (badgeId) => {
    ...
    fetch("https://api.alexsama.tech/api/users/badge-unlock", {
      ...
      const apiQuery = forkJoin([fetchBadges(), fetchUserDetails()]).subscribe(
        ([unlockedBagesRes, userDetailRes]) => {
    ...
    <Fragment>
      {isSignedIn ? (
        <div className="flex flex-col pt-5 w-full">
          <div className="text-5xl text-center text-white">
            REWARDS (AVAILABLE POINTS: {userPoints})
          </div>
          <div className="grid grid-cols-4 gap-5 p-10 w-full">
            {badges.map((badge, idx) =>
              unlockedBadgeIds.includes(badge.id) ? (
                <div
                  className="h-[600px] bg-white border-[40px] border-red-500"
                  key={`badge-${idx}`}
                >
                  <div className="flex justify-between items-center mt-[-40px] h-[40px]">
                    <div className="text-white font-bold text-xl w-[200px] text-ellipsis whitespace-nowrap overflow-hidden">
                      PIZZA BOI
                    </div>
                ...
                <div
                  key={`badge-${idx}`}
                  className="flex flex-col gap-5 justify-center items-center h-[600px] bg-black text-white border-4 border-white"
                >
      ...
      <Dialog open={errorDialogOpen}>
        <div className="h-[450px] w-[500px] flex flex-col items-center gap-10">
          ...
          <button
            className="p-5 mt-auto bg-red-500 text-xl text-white w-full"
        ...
        {dialogOpen &&
          (() => {
            const selectedBadge = _.find(
              badges,
              (badge) => badge.id === selectedBadgeIdToUnlock
            );
            return (
